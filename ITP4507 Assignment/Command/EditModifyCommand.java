...
public class EditModifyCommand implements Command {
    Vector<Building> builde;
    Scanner input;
    private int id, nor, buil, mRoom;
    private double mLength, mWidth;

    public EditModifyCommand(Vector<Building> builde, int id, int nor, Scanner input) {
        this.builde = builde;
        this.id = id;
        this.nor = nor;
        this.input = input;
        ...
        for (int i = 0; i < builde.size(); i++) {
            if (builde.elementAt(i).getId() == EditCommandFactory.bID) {
                if (builde.elementAt(i) instanceof Apartment) {
                    ...
                    mLength = input.nextDouble();

                    System.out.print("Width: ");
                    mWidth = input.nextDouble();

                    System.out.println("Updated Building:");
                    ((Apartment) builde.elementAt(i)).modifyRoom(mRoom - 1, mLength, mWidth);
                    ...
                    System.out.println("Updated Building:");
                    ((House) builde.elementAt(i)).modifyRoom(mRoom - 1, mLength, mWidth);
